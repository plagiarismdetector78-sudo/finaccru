...
const BillLayoutP2 = ({ items, setItems, notes, setNotes, currency, currencies,
    ...
    const handleInputChange = (index, key, value) => {
        const updatedItems = items?.map((item) => ({ ...item }));
        updatedItems[index][key] = value;
        if (key === 'rate' || key === 'discount') {
            updatedItems[index][key] = Number(value).toFixed(2);
        }
        setItems(updatedItems);
    };
                        ...
                        <div className='layout__items--whole-item'>
                            ...
                            <div className='layout__items--number-item'>
                                ...
                                <Input
                                    ...
                                    defaultValue={Number(item?.rate).toFixed(2)}
                                    style={{
                                        width: 70
                                    }}
                                    onChange={(e) => {
                                        const valid = e.target.value.match(/^\d*\.?\d{0,2}$/);
                                        if (valid) {
                                            handleInputChange(index, 'rate',Number(e.target.value).toFixed(2));
                                        }
                                    }}
